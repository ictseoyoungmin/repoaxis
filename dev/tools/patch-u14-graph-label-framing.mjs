...
const newBadge=`function badgeSvg(code,x,y,w=18,h=15){if(!code)return'';const [f,s,t]=colorFor(code),label=statusLabel(code),actualW=Math.max(w,10+label.length*5),drawX=x+w-actualW;return\`<g><rect x="\${drawX}" y="\${y}" width="\${actualW}" height="\${h}" rx="5" fill="\${f}" stroke="\${s}"/><text x="\${drawX+actualW/2}" y="\${y+h/2+3}" text-anchor="middle" font-size="8" font-weight="700" fill="\${t}">\${esc(label)}</text></g>\`}`;
...
const test=`import assert from "node:assert/strict";\nimport fs from "node:fs";\nimport test from "node:test";\nimport { fileURLToPath } from "node:url";\n\nconst V0=fileURLToPath(new URL("../../../skills/repoaxis/viewer/viewer-0.js",import.meta.url));\nconst V2=fileURLToPath(new URL("../../../skills/repoaxis/viewer/viewer-2.js",import.meta.url));\nconst v0=fs.readFileSync(V0,"utf8"),v2=fs.readFileSync(V2,"utf8");\n\ntest("Git badges widen inward for multi-character labels",()=>{\n  assert.ok(v0.includes("actualW=Math.max(w,10+label.length*5)"));\n  assert.ok(v0.includes("drawX=x+w-actualW"));\n  assert.ok(v0.includes('x="\\${drawX}"'));\n});\n\ntest("impact Graph entry frames all projected roots instead of only the selected root",()=>{\n  assert.ok(v2.includes("projection.mode==='impact'&&projection.list?.length"));\n  assert.ok(v2.includes("projection.list.map(f=>L.pos[f.id]).filter(Boolean)"));\n  assert.ok(v2.includes("cx=(minX+maxX)/2,cy=(minY+maxY)/2"));\n  assert.ok(v2.includes("fit=Math.min(s0,(L.viewW-160)/contentW,(L.viewH-150)/contentH)"));\n});\n`;
