...
try{
  await page.goto(viewer.url,{waitUntil:'domcontentloaded'});await page.waitForTimeout(5000);
  const status=await page.evaluate(()=>({live:document.documentElement.dataset.repoaxisLive||null,adapter:window.__REPOAXIS_LIVE__?{ready:window.__REPOAXIS_LIVE__.ready,error:window.__REPOAXIS_LIVE__.error}:null,repo:document.querySelector('.repo-name')?.textContent,branch:document.querySelector('.branch > span:first-of-type')?.textContent,sha:document.querySelector('.commit code')?.textContent}));
  if(status.live!=='ready'){
    await page.screenshot({path:path.join(out,'failure-1600.png')});const failure={status,errors};fs.writeFileSync(path.join(out,'metrics.json'),JSON.stringify(failure,null,2));console.log(JSON.stringify(failure));process.exitCode=1;return;
  }
  await page.waitForTimeout(500);
  const metrics=await page.evaluate(()=>({live:document.documentElement.dataset.repoaxisLive,repo:document.querySelector('.repo-name')?.textContent,host:document.querySelector('.public')?.textContent,branch:document.querySelector('.branch > span:first-of-type')?.textContent,sha:document.querySelector('.commit code')?.textContent,detail:window.__REPOAXIS_LIVE__&&{nodes:tree.length-1,files:files().length,imports:importEdges.length,changes:changes.length,depRoot:state.depRoot},fixtureRepo:document.querySelector('.commit code')?.textContent==='a1b2c3d',overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth}));
  ...
  if(metrics.fixtureRepo||!metrics.detail?.files||!metrics.detail?.imports||!surfaces.structureNodes||!surfaces.depNodes||!surfaces.graphNodes||errors.length)process.exitCode=1;
