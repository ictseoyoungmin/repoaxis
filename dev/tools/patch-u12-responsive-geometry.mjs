...
v0=replaceOrFail(v0,
"camera:{structure:{s:1,x:0,y:0},dependencies:{s:1,x:0,y:0},graph:{s:1,x:0,y:0}},fingerprint:null",
"camera:{structure:{s:1,x:0,y:0},dependencies:{s:1,x:0,y:0},graph:{s:1,x:0,y:0}},spatialViewport:{},fingerprint:null",
...
v1=replaceOrFail(v1,
'<svg class="dep-svg" id="depSvg" viewBox="0 0 ${W} ${H}">${sh}</svg>',
'<svg class="dep-svg" id="depSvg" viewBox="0 0 ${vp.w} ${vp.h}">${sh}</svg>',
'dependency responsive viewBox');
...
"  const W=Math.max(GRAPH_VIEWPORT.w,maxX+40,vp.w),H=Math.max(GRAPH_VIEWPORT.h,cursorY+rowH+100,vp.h);return{pos,scopes,W,H,viewW:vp.w,viewH:vp.h}",
'graph responsive viewport return');
v3=replaceOrFail(v3,
"if(state.selected===ROOT&&sel)state.selected=sel.id;svg.setAttribute('viewBox',`0 0 ${L.viewW} ${L.viewH}`);",
"if(state.selected===ROOT&&sel)state.selected=sel.id;reconcileSpatialViewport('graph',{w:L.viewW,h:L.viewH});svg.setAttribute('viewBox',`0 0 ${L.viewW} ${L.viewH}`);",
'graph camera reconciliation');
