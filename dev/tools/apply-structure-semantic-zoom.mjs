...
function semanticStructureLayout(){
  ...
  semanticStructurePositions=pos;semanticStructureWorld={W,H,viewW,viewH,fit};overviewPositions=pos;return semanticStructureWorld;
}
...
function semanticStructureDetailSet(id){
  const selected=byId[id]||byId.root,set=new Set(semanticStructurePath(selected.id)),add=n=>{if(n)set.add(n.id)};
  ...
  while(queue.length&&count<42){const q=queue.shift(),n=q.n,d=q.d;if(d>=2)continue;for(const c of children(n.id)){if(count>=42)break;if(!set.has(c.id)){set.add(c.id);count++}if(c.type==='folder'||c.type==='file')queue.push({n:c,d:d+1})}}
  if(selected.id==='root')children('root').slice(0,22).forEach(add);return set;
}
...
function semanticStructureGlyph(n,x,y){
  const isRoot=n.id==='root',stroke=isRoot?'#625bff':n.type==='folder'?'#8a82ff':'#cdd3df';
  if(isRoot||n.type==='folder')return(isRoot?'<circle class="root-halo" cx="'+x+'" cy="'+y+'" r="18"/>':'')+'<rect class="semantic-glyph-shape" x="'+(x-7)+'" y="'+(y-7)+'" width="14" height="14" rx="4" fill="#fff" stroke="'+stroke+'" stroke-width="'+(isRoot?2:1.4)+'"/>';
  return'<circle class="semantic-glyph-shape" cx="'+x+'" cy="'+y+'" r="'+(n.type==='function'?5.5:4.8)+'" fill="'+(n.type==='function'?'#f7f6ff':'#fff')+'" stroke="'+stroke+'" stroke-width="1.2"/>';
}
...
function renderSemanticStructureOverview(){
  const world=semanticStructureLayout(),detailPath=new Set(semanticStructurePath(state.selected)),selected=byId[state.selected]||byId.root;$('#overviewSvg').setAttribute('viewBox','0 0 '+world.viewW+' '+world.viewH);
...
function frameSemanticStructureSelection(id){semanticStructureLayout();const p=semanticStructurePositions[id];if(!p)return;const w=semanticStructureWorld,s=Math.max(1,Math.min(1.18,1/Math.max(.001,w.fit)*.42));state.cameras.structureOverview={s:s,x:w.viewW/2-p[0]*s,y:w.viewH/2-p[1]*s};applyCamera('structureOverview');updateSemanticStructureLod()}
...
const bootRepositoryBeforeSemanticZoom=bootRepository;bootRepository=async function(){await bootRepositoryBeforeSemanticZoom();if(repositoryRuntime.ready&&state.view==='structure'&&state.mode==='overview'){renderSemanticStructureOverview();fitSemanticStructure()}};
`;
